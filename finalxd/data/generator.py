...
from model_classes import Owner, Car, History  # Updated import path to match model_classes.py
import random
...
def generate_owner(n: int, male_ratio: float = 0.5,
    ...
    for i in range(n):
        ...
        owner_id = f"P-{str(i + 1).zfill(6)}"  # Generating unique owner ID
...
def generate_cars(n: int, automatic_ratio: float = 0.2,
    ...
    for i in range(n):
        ...
        car_type = fake.vehicle_make()  # Assuming type here refers to the vehicle make
...
def generate_history(cars: list[Car], owners: list[Owner], n: int,
    ...
    for i in range(n):
        transaction_id = f"T-{str(i + 1).zfill(6)}"  # Unique transaction ID
        car = random.choice(cars)                    # Randomly select a car
        owner = random.choice(owners)                # Randomly select an owner
        purchase_date = fake.date_between(start_date='-10y', end_date='today')  # Purchase within last 10 years
        mileage = str(random.randint(min_mileage, max_mileage))  # Random mileage within range
